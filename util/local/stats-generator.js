...
function getLanguageNameFromDirPath(dirPath) {
	for (const lang in languageModel) {
		for (const dirName of languageModel[lang].dirNames) {
...
function generatetotalProblemCounterAndUpdateStatsArray(statsArr) {
	...
	for (const obj of statsArr) {
		...
		for (const lang in counter) {
			...
			if (languageModel[lang].types.includes("database")) {
				type = 'database';
			} else if (config.JS_TS_QUES_IDS.includes(quesId)) {
	...
	return { totalProblemCounter, totalLanguageCounter, fileCounter };
}
...
async function updateStatsinReadmeFile(totalProblemCounter, totalLanguageCounter) {
	return new Promise(async (resolve, reject) => {
		try {
...
async function generateStats() {
	try {
		...
		logger.time('Solutions Stat Generation Started.');

		const statsMap = await generateStatsMap();

		const statsArr = convertMapToArray(statsMap);
		logger.log(`stats array length = ${statsArr.length}`)
		logger.log(`stats for index 0 = ${JSON.stringify(statsArr[0])}`)

		const { totalProblemCounter, totalLanguageCounter, fileCounter } =
			generatetotalProblemCounterAndUpdateStatsArray(statsArr);

		logger.log(`total problem counter = ${JSON.stringify(totalProblemCounter)}`);
		logger.log(`total language counter = ${JSON.stringify(totalLanguageCounter)}`);
		logger.log(`file counter = ${JSON.stringify(fileCounter)}`);
		...
		// await updateStatsinReadmeFile(totalProblemCounter, totalLanguageCounter);

		logger.time('Solutions Stat Generation Completed.');
