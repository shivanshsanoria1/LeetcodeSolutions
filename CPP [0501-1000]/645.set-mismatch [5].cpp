...
public:
    ...
    vector<int> findErrorNums(vector<int>& nums) { 
        int n=nums.size();
        int sum1 = 0;
        int correctSum1 = (n*(n+1))/2;
        long long int sum2 = 0;
        long long int correctSum2 = ((long long int)n*(n+1)*(2*n+1))/6;
        ...
        int miss_minus_dup = correctSum1 - sum1;
        int miss_plus_dup = (correctSum2 - sum2) / (correctSum1 - sum1);
